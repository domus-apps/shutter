#!/bin/bash
# Builds a standalone Shutter.app at build/Shutter.app.
set -euo pipefail
cd "$(dirname "$0")/.."

swift build -c release

APP=build/Shutter.app
rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp .build/release/Shutter "$APP/Contents/MacOS/Shutter"
...
# Sources/Shutter/Localization.swift — the generated accessor can't be
# trusted across toolchains).
cp -R .build/release/Shutter_Shutter.bundle "$APP/Contents/Resources/"
...
# Otherwise fall back to ad-hoc, which is fine locally — Shutter needs no
